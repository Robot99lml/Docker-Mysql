...
class CIT(db.Model):
    ...
    def get_response(self, **kwargs):
        ...
        entity = {
            ...
            'FECHASOL': self.FECHASOL and self.FECHASOL.isoformat(timespec='seconds'),
            'FECHA': self.FECHA and self.FECHA.isoformat(timespec='seconds'),
            'FECHAATENCION': self.FECHAATENCION and self.FECHAATENCION.isoformat(timespec='seconds'),
            ...
            'IDSERVICIOPROC': self.IDSERVICIOPROC,
            'ATENCION': self.ATENCION,
            'PVEZ': self.PVEZ,
            'IDCONTRATANTE': self.IDCONTRATANTE,
            'IDSEDE': self.IDSEDE,
            'IDMEDICO': self.IDMEDICO,
            'IDPLAN': self.IDPLAN,
            'URGENCIA': self.URGENCIA,
            'VALORTOTAL': self.VALORTOTAL,
            'VALORCOPAGO': self.VALORCOPAGO,
            'VALOREXEDENTE': self.VALOREXEDENTE,
            'VALORTOTALCOS': self.VALORTOTALCOS,
            'VALORCOPAGOCOSTO': self.VALORCOPAGOCOSTO,
            'IMPRESO': self.IMPRESO,
            'CUMPLIDA': self.CUMPLIDA,
            'IDPESPECIAL': self.IDPESPECIAL,
            'IDESTADOE': self.IDESTADOE,
            'ESTADO': self.ESTADO,
            'CAUSALBLOQUEO': self.CAUSALBLOQUEO,
            'GENEROCAJA': self.GENEROCAJA,
            'IDSEDEATENCION': self.IDSEDEATENCION,
            'TELEFONOAVISO': self.TELEFONOAVISO,
            'IDCAUSAL': self.IDCAUSAL,
            'DISPONIBILIDAD': self.DISPONIBILIDAD,
            'SPD': self.SPD,
            'NORECIBOCAJA': self.NORECIBOCAJA,
            'CLASEORDEN': self.CLASEORDEN,
            'USUARIO': self.USUARIO,
            'NOADMISION': self.NOADMISION,
            'GENPRESTACION': self.GENPRESTACION,
            'FINCONSULTA': self.FINCONSULTA,
            'CODCAJA': self.CODCAJA,
            'FECHALLEGA': self.FECHALLEGA and self.FECHALLEGA.isoformat(timespec='seconds'),
            'USUARIOLLEGA': self.USUARIOLLEGA,
            'OBSERVACIONES': self.OBSERVACIONES,
            'NOAUTORIZACION': self.NOAUTORIZACION,
            'IDSEDEORIGEN': self.IDSEDEORIGEN,
            'IDIPSSOLICITA': self.IDIPSSOLICITA,
            'IDMEDICOSOLICITA': self.IDMEDICOSOLICITA,
            'AUTORIZADOPOR': self.AUTORIZADOPOR,
            'FECHAAUTORIZACION': self.FECHAAUTORIZACION and self.FECHAAUTORIZACION.isoformat(timespec='seconds'),
            'CNSAFIAA': self.CNSAFIAA,
            'TIPOCOPAGO': self.TIPOCOPAGO,
            'PROCEDENCIA': self.PROCEDENCIA,
            'IDAREAH': self.IDAREAH,
            'IDAREA': self.IDAREA,
            'CCOSTO': self.CCOSTO,
            'SUBCCOSTO': self.SUBCCOSTO,
            'NIVELATENCION': self.NIVELATENCION,
            'FACTURADA': self.FACTURADA,
            'N_FACTURA': self.N_FACTURA,
            'CNSFCT': self.CNSFCT,
            'VFACTURAS': self.VFACTURAS,
            'FACTURABLE': self.FACTURABLE,
            'DESCUENTO': self.DESCUENTO,
            'TIPODTO': self.TIPODTO,
            'MARCAFAC': self.MARCAFAC,
            'IDIPS': self.IDIPS,
            'CLASECONTRATO': self.CLASECONTRATO,
            'ENPAQUETE': self.ENPAQUETE,
            'IDCIRUGIA': self.IDCIRUGIA,
            'CNSFACT': self.CNSFACT,
            'SOAT': self.SOAT,
            'COPAGOPROPIO': self.COPAGOPROPIO,
            'CNSHACTRAN': self.CNSHACTRAN,
            'ALTOCOSTO': self.ALTOCOSTO,
            'IDDX': self.IDDX,
            'IDEMEDICA': self.IDEMEDICA,
            'IDTERCEROCA': self.IDTERCEROCA,
            'FINALIDAD': self.FINALIDAD,
            'TIPOCAJA': self.TIPOCAJA,
            'CONSECUTIVOHCA': self.CONSECUTIVOHCA,
            'AQUIENCOBRO': self.AQUIENCOBRO,
            'VALORPROV': self.VALORPROV,
            'CODUNG': self.CODUNG,
            'CODPRG': self.CODPRG,
            'NOCITA': self.NOCITA,
            'REASIGNADA': self.REASIGNADA,
            'IDCONTRATO': self.IDCONTRATO,
            'ITFC': self.ITFC,
            'CNSITFC': self.CNSITFC,
            'SYS_COMPUTERNAME': self.SYS_COMPUTERNAME,
            'NOCOBRAR': self.NOCOBRAR,
            'CCONTRATANTE': self.CCONTRATANTE,
            'CONTABILIZADA': self.CONTABILIZADA,
            'NROCOMPROBANTE': self.NROCOMPROBANTE,
            'MARCACONT': self.MARCACONT,
            'QXCONSECUTIVO': self.QXCONSECUTIVO,
            'FECHACIRUGIA': self.FECHACIRUGIA and self.FECHACIRUGIA.isoformat(timespec='seconds'),
            'FECHAREASIG': self.FECHAREASIG and self.FECHAREASIG.isoformat(timespec='seconds'),
            'USUARIOREASIG': self.USUARIOREASIG,
            'CODCUPS': self.CODCUPS,
            'IDPLAN_AFI': self.IDPLAN_AFI,
            'IDTERCERO_AFI': self.IDTERCERO_AFI,
            'NOMBREACOMPA': self.NOMBREACOMPA,
            'TELEFONOACOMPA': self.TELEFONOACOMPA,
            'PARENTESCOACOMPA': self.PARENTESCOACOMPA,
            'NOMBRERESPO': self.NOMBRERESPO,
            'TELEFONORESPO': self.TELEFONORESPO,
            'PARENTESCORESPO': self.PARENTESCORESPO,
            'USUARIONOFACTURABLE': self.USUARIONOFACTURABLE,
            'FECHA_NOFAC': self.FECHA_NOFAC and self.FECHA_NOFAC.isoformat(timespec='seconds'),
            'PVEZANO': self.PVEZANO,
            'F_REQUERIDA': self.F_REQUERIDA,
            'RIESGO': self.RIESGO,
            'DURACION': self.DURACION,
            'MVARIAS': self.MVARIAS,
            'NROCITAS': self.NROCITAS,
            'CNSORIGINAL': self.CNSORIGINAL,
            'PREGUNTAPOR': self.PREGUNTAPOR,
            'MPLAN': self.MPLAN,
            'IDPLANDISP': self.IDPLANDISP,
            'WEB': self.WEB,
            'DIRECCION': self.DIRECCION,
            'CIUDAD': self.CIUDAD,
            'CORREGIMIENTO': self.CORREGIMIENTO,
            'LAT_DIR': self.LAT_DIR,
            'LNG_DIR': self.LNG_DIR,
            'IDBARRIO': self.IDBARRIO,
            'MODALIDAD': self.MODALIDAD,
            'TIPOAFILIADO': self.TIPOAFILIADO,
            'IDAUTSE': self.IDAUTSE,
            'COVID': self.COVID,
            'USUATENCION': self.USUATENCION,
            'USURESPONSABLE': self.USURESPONSABLE,
            'USUGENAGENDA': self.USUGENAGENDA,
            'FGENERACION': self.FGENERACION,
            'IDKPAGE': self.IDKPAGE,
            'COMERCIAL': self.COMERCIAL,
            'HORADESDE': self.HORADESDE,
            'HORAHASTA': self.HORAHASTA,
            'RESULTADOLLAMADA': self.RESULTADOLLAMADA,
            'USUPREANALITICA': self.USUPREANALITICA,
            'FECHAPREANALITICA': self.FECHAPREANALITICA and self.FECHAPREANALITICA.isoformat(timespec='seconds'),
            'FECHAENVIOLAB': self.FECHAENVIOLAB and self.FECHAENVIOLAB.isoformat(timespec='seconds'),
            'ESTADOPREANALITICA': self.ESTADOPREANALITICA,
            'USUNOPROCESA': self.USUNOPROCESA,
            'FECHANOPROCESA': self.FECHANOPROCESA and self.FECHANOPROCESA.isoformat(timespec='seconds'),
            'RAZONNOPROCESA': self.RAZONNOPROCESA,
            'ESPROGRAMAS': self.ESPROGRAMAS,
            'URLVIDEOLLAMADA': self.URLVIDEOLLAMADA,
            'CANTIDADC': self.CANTIDADC,
        }
        return entity